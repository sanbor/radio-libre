...
- **What could be improved** — List 3–5 specific, actionable improvements across user experience, code quality, performance, and test coverage. Be concrete (e.g., "The error state in StationListView shows a generic message — could show network vs. server errors distinctly" or "The search debounce is 300ms but feels sluggish with large lists — consider 150ms"). Focus on improvements that would meaningfully impact the user or developer experience, not nitpicks. After listing them, ask the user: *"Would you like me to add any of these to the **Potential improvements** section in PLAN.md so they're tracked for future work?"*
- **Best next steps** — Based on SPEC.md and PLAN.md, recommend 3–5 valuable next `/implement` commands the user could run, ordered by impact. Consider: what unblocked features depend on what was just built? What would deliver the most user-facing value? What is the natural continuation of the current work? Format each as a concrete suggestion: *"`/implement <feature>` — <why it's valuable and what it builds on>"*
